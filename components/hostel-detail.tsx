            ...
            <div className="grid gap-12 lg:grid-cols-[1fr_400px] lg:gap-16">
                ...
                <aside className="lg:sticky lg:top-24 lg:self-start">
                    <div className="overflow-hidden rounded-sm border-2 border-[#0c4da2] bg-white">
                        <div className="border-b-2 border-[#0c4da2] px-4 py-3 sm:px-5 sm:py-4 md:px-6 md:py-5">
                            <h2 className="font-display text-lg font-medium text-stone-900 sm:text-xl md:text-2xl">
                                Pricing Summary
                            </h2>
                        </div>

                        <div className="space-y-4 px-4 py-4 sm:px-5 sm:py-5 md:space-y-5 md:px-6 md:py-6">
                            <PriceRow
                                label="Selected Room"
                    ...
                    <span className="text-base font-medium text-stone-800 sm:text-lg">
                      Laundry
                    </span>
                    <span className="mt-0.5 block text-sm text-stone-500">
                      Optional
                                ...
                                <span
                                    className={cn(
                                        'tabular-nums transition-colors text-base sm:text-lg',
                                        !laundryEnabled
                  ...
                  ₹{hostel.laundryFees.toLocaleString('en-IN')}
                        ...
                        <div className="border-t-2 border-[#0c4da2] bg-[#0c4da2]/[0.05] px-4 py-4 sm:px-5 sm:py-5 md:px-6">
                            <div className="flex items-baseline justify-between gap-3">
                <span className="font-mono text-[11px] uppercase tracking-[0.2em] text-stone-700 md:text-xs">
                  Total / Year
                </span>
                                <span className="font-display text-2xl font-medium leading-none tabular-nums text-[#0c4da2] sm:text-[1.75rem] md:text-[2rem]">
                  ₹{total.toLocaleString('en-IN')}
                        ...
                        <div className="p-3">
                            <Button
                                ...
                                render={<Link href="/redirect-notice" />}
                            >
                                Proceed to Booking
            ...
            <div className="grid gap-x-16 gap-y-10 border-t border-stone-300 pt-12 md:grid-cols-2">
                <section className="space-y-5">
                    ...
                    <dl className="space-y-3 text-base">
                        <ContactRow label="Name" value={hostel.wardenName} />
                        <ContactRow
                            label="Phone"
...
function RoomOption({
    ...
    return (
        <button
            ...
            className={cn(
                'group relative block w-full rounded-sm border-2 px-4 py-4 text-left transition-all duration-150 sm:px-5 sm:py-5 md:px-6 md:py-6',
                selected
        ...
        >
            {isUrlMatch && (
                <span className="absolute -top-2.5 left-4 bg-white px-2 font-mono text-[10px] font-extrabold uppercase tracking-[0.2em] text-[#0c4da2] sm:left-5">
          Your Preference
        </span>
            )}
            <div className="flex items-start justify-between gap-3 sm:gap-6">
                <div className="flex min-w-0 flex-1 items-start gap-3 sm:gap-4">
                    ...
                    <span
                        className={cn(
                            'mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full border-2 transition-colors sm:mt-1',
                            selected
            ...
            {selected && (
                <Check className="size-3 text-white" strokeWidth={3} />
                    ...
                    <div className="min-w-0 flex-1 space-y-1.5">
                        <div className="flex flex-wrap items-center gap-x-1.5 gap-y-1 font-display text-base font-medium text-stone-900 sm:text-lg md:text-xl">
              <span className="inline-flex items-center gap-1.5">
                ...
                {room.washroom === 'attached' ? 'Attached' : 'Common'} Washroom
                        ...
                        {room.notes && (
                            <p className="inline-flex items-start gap-1.5 rounded-sm bg-amber-50 px-2 py-1 text-xs leading-snug text-amber-900 ring-1 ring-amber-200 sm:text-sm">
                ...
                <div className="shrink-0 text-right">
                    <p
                        className={cn(
                            'font-display text-lg font-medium tabular-nums sm:text-xl md:text-2xl',
                            selected ? 'text-[#0c4da2]' : 'text-stone-900'
                        )}
                    >
                        ₹{room.price.toLocaleString('en-IN')}
                    </p>
                    <p className="text-[10px] text-stone-500 sm:text-xs">room only</p>
                </div>
            </div>
        </button>
    )
    ...
    return (
        <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
                ...
                {sublabel && (
                    <p className="mt-0.5 text-sm text-stone-500">{sublabel}</p>
                )}
            </div>
            <span className="shrink-0 text-base font-medium tabular-nums text-stone-900 sm:text-lg">
